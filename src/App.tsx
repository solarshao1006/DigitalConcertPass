...
import {
  ...
  Concert,
  PRICE_TIERS,
  getCurrencyForConcert,
  getPriceTiersForConcert,
} from './constants';

const FALLBACK_IMAGE = 'https://picsum.photos/seed/layzhang/800/1200';
const DRAMA_BACKGROUND_URL = '/imgs/backgroud_dunhuang.jpg';
const DRAMA_POSTER_URL = '/imgs/dunhuang/poster.jpg';
...
const isDramaConcert = (concert: Concert) => concert.type === 'drama';

const getConcertImage = (concert: Concert) => {
  if (isDramaConcert(concert)) {
    return DRAMA_POSTER_URL;
  }

  const season = concert.season || 5;
  const pinyin = CITY_PINYIN_MAP[concert.city] || 'beijing';
...
const getConcertBackgroundImage = (concert: Concert) => {
  if (isDramaConcert(concert)) {
    return DRAMA_BACKGROUND_URL;
  }

  return `/imgs/grandline${concert.season || 5}_bg.jpg`;
};
...
export default function App() {
  ...
  const handleDownloadPass = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `ticket_${selectedConcert.id}_${Date.now()}.pkpass`;
            ...
            >
                        ...
                        >
                          {availablePriceTiers.map((p) => (
                            <option key={p} value={p} className="bg-zinc-900">
                              {getCurrencyForConcert(selectedConcert!)}
                              {p}
                            </option>
                          ))}
                        </select>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => {
                            const concert = CONCERTS.find((item) => item.id === e.target.value);
                        ...
                        >
                          {CONCERTS.map((concert) => (
                            <option key={concert.id} value={concert.id} className="bg-zinc-900">
                              {concert.date} · {concert.city} · {getConcertLabel(concert)}
                            </option>
                          ))}
                        </select>
              ...
              <footer className="mt-auto pt-12 text-center">
                ...
                <p className="mt-2 text-[11px] text-zinc-500">
                  Solar |{' '}
                  <a
                    href="https://github.com/solarshao1006"
                    target="_blank"
                    rel="noreferrer"
                    className="underline underline-offset-4"
                  >
                    github.com/solarshao1006
                  </a>
                </p>
              ...
              <div className="px-6 py-8 flex items-center justify-between">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs font-bold text-white">
                    {selectedConcert ? getConcertStopLabel(selectedConcert) : ''}
                  </p>
                </div>
                <div className="w-12" />
              </div>
              ...
              <div className="flex-1 flex items-center justify-center px-6 py-4">
                <div className="w-full flex justify-center">
                  <TicketView
                    ...
                    seat={seat}
                    location={location}
                    currency={getCurrencyForConcert(selectedConcert!)}
                  />
                </div>
              </div>
...
function TicketView({
  ...
  const fullDate = concert.date;
  const displayDate = fullDate.replace(/-/g, ' / ');
  const isDrama = isDramaConcert(concert);
  const ticketTitle = getConcertLabel(concert);
  const englishLabel = getConcertEnglishLabel(concert);
  const subLabel = getConcertSubLabel(concert);
  const idPrefix = isDrama ? 'DH2025' : 'LZ2025';
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        <img
          src={getConcertBackgroundImage(concert)}
          alt="Background"
      ...
      <div className="relative z-20 px-6 pt-6 flex items-start justify-between">
        ...
        <div className="text-right space-y-1.5 max-w-[120px]">
          <p className="text-[8px] font-black text-white leading-none whitespace-nowrap drop-shadow-md">
            {fullDate.replace(/-/g, '.')} {concert.time}
          </p>
          <p className="text-[7px] font-bold text-purple-100 tracking-tight leading-tight drop-shadow-md">
      ...
      <div className="relative z-20 px-8 pb-8 flex flex-col justify-between bg-[#4c1d95]/60">
        ...
        <div className="mt-8 pt-6 border-t border-white/20 flex flex-col items-center">
          <p className="text-[8px] font-mono text-purple-200/60 tracking-[0.4em] uppercase">
            {userName ? userName.replace(/\s/g, '_') : 'GUEST'}_{idPrefix}_{concert.id}
          </p>
          <p className="text-[6px] text-purple-200/40 mt-2 tracking-widest uppercase italic">
            All copyrights reserved | Solar
          </p>
        </div>
      </div>
    </motion.div>
